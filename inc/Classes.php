...
$section_tags=array();
$section_tags['target_owl_url'] = '[Online URL of the target OWL file (RDF/XML format)]';
$section_tags['data_file_url'] = '[Online URL of the data file (Only Excel file (.xls, .xlsx) or tab-delimited text file (.txt)]';
$section_tags['cls_col'] = '[New axioms will be used to]';
$section_tags['anno_tpl'] = '[Annotations]';
$section_tags['equi_tpl'] = '[Equivalent classes]';
$section_tags['sup_tpl'] = '[Superclasses]';
$section_tags['extra_terms'] = '[Terms used to define anntoations, equivalent classes and superclasses]';
$section_tags['term_uri_start_with'] = '[Term URIs start with]';
$section_tags['auto_id'] = '[Auto-generated term ID]';



$ref_types = array('Class', 'ObjectProperty', 'DatatypeProperty');
...
function convertIntToAlphabet($int_wert, $lowercase=true) {
	$base = $lowercase ? 96 : 64;
	$alpha_string='';
	$int_wert--;
	if($int_wert/26>=1) {
		$alpha_string=chr(($int_wert%26)+$base+1).$alpha_string;
		$alpha_string=convertIntToAlphabet($int_wert/26).$alpha_string;
	}
	else {
		$alpha_string=chr($int_wert+$base+1);
	}
...
function getSettings($ontology_abbrv) {
	...
	$settings['pattern_main'] = 'http://';
	
	$settings['imports'] = array();
	
	$settings['core_terms'] = array();
	$strSql= "select * from key_terms where ontology_abbrv='$ontology_abbrv'";
	$results = $db->GetAll($strSql);
	
	if ($results!=false) {
		foreach($results as $result) {
			if ($result['is_root']==1) {
				$settings['core_terms'][$result['term_url']]='Root class: '.$result['term_label'];
			}
			else {
	...
	$strSql= "select * from import where ontology_abbrv='$ontology_abbrv'";
	$results = $db->GetAll($strSql);
	
	if ($results!=false) {
		foreach($results as $result) {
			if ($result['import_url']!='http://protege.stanford.edu/plugins/owl/dc/protege-dc.owl') {
				$import_graph_url=$result['import_graph_url']!=='' ? $result['import_graph_url'] : $result['import_url'];
				$settings['imports'][$result['import_url']]=array('graph'=>$import_graph_url, 'pattern'=>'http://');
			}
		}
	}
	...
	if(isset($result_main[$tmp_iri])) {
		foreach ($result_main[$tmp_iri] as $result) {
			if ($link && $result['type']=='uri') {
				$array_return[]="<a href=\"rdf.php?iri=" . myUrlEncode($result['value']) . "\">" . $a_label[$result['value']] . "</a>";
			}
...
function printRDFjsonEquivalent($obj_equiv, $a_label, $relation = 'and', $last_re='and', $first_re=true) {
	...
	if(!is_array($v[0])) {
		if ($v[0]!='http://www.w3.org/1999/02/22-rdf-syntax-ns#nil') $strOut .= '<a href="rdf.php?o='.$o.'&amp;iri=' . myUrlEncode($v[0]) . '">' . $a_label[$v[0]] . '</a>';
	}
	...
	if (isset($v[1])) {
		if(!is_array($v[1])) {
			if ($v[1]!='http://www.w3.org/1999/02/22-rdf-syntax-ns#nil') {
				$strOut .= " $relation " . '<a href="rdf.php?o='.$o.'&amp;iri=' . myUrlEncode($v[1]) . '">' . $a_label[$v[1]] . '</a>';
			}
		}
