...
$e = $vali->getInput('e', 'Example', 0, 2);

$showEg=false;
if ($e==1) {
	include('examples/eg1.php');
	$showEg=true;
}
...
<form action="domapping.php" method="post" enctype="multipart/form-data" target="_blank">
<table border="0">
  <tr>
    <td><h3 class="header_darkred">Ontorat: Ontology Representation of Axioms using Templates</h3>
  ...
  <tr>
    <td style="padding-left:16px">Online URL:
      <input name="target_owl_url" id="target_owl_url" value="<?php echo $showEg ? $target_owl_url : ''?>" size="70"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td style="padding-left:16px">Online URL:
      <input name="data_file_url" id="data_file_url" value="<?php echo $showEg ? $data_file_url : ''?>" size="70"></td>
  </tr>
  <tr>
...
for ($i=1; $i<=5; $i++) {
?>
    <option value="<?php echo $i?>" <?php  if (isset($data_start_row) && $data_start_row==$i) {?> selected <?php  }?>><?php echo $i?></option>
    <?php 
}
?>
  </select>
  ...
  <tr>
    <td style="padding-left:16px"><select name="cls_col" id="cls_col">
        <option value="new">generate new classes</option>
      <?php 
for ($i=1; $i<=26; $i++) {
?>
      <option value="{$column<?php echo strtoupper(convertIntToAlphabet($i))?>}">edit existing classes with term IDs defined in column
...
?>
  ...
  <tr>
    <td style="padding-left:16px"><textarea name="anno_tpl" cols="90" rows="5" id="anno_tpl" wrap="off"><?php echo $showEg ? $anno_tpl : ''?></textarea></td>
  </tr>
  <tr>
  ...
  <tr>
    <td style="padding-left:16px"><textarea name="equi_tpl" cols="90" rows="5" id="equi_tpl"></textarea></td>
  </tr>
  <tr>
...
<option value="label">term label</option>
  ...
  <tr>
    <td style="padding-left:16px"><textarea name="sup_tpl" cols="90" rows="6" id="sup_tpl" wrap="off"><?php echo $showEg ? $sup_tpl : ''?></textarea></td>
  </tr>
  <tr>
...
<input name="extra_term_uri" id="extra_term_uri" value="http://pur.obolibrary.og/obo/" size="50">
  ...
  <tr>
    <td style="padding-left:16px"><textarea name="extra_terms" cols="90" rows="8" id="extra_terms" wrap="off"><?php echo $showEg ? $extra_terms : 'AnnotationProperty: 
...
'?></textarea></td>
  ...
  <tr>
    <td style="padding-left:16px">Start with:
      <input name="term_uri_start_with" id="term_uri_start_with" value="http://purl.obolibrary.org/obo/" size="80"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td style="padding-left:16px">Prefix:
      <input name="term_id_prefix" id="term_id_prefix" value="<?php echo $showEg ? $term_id_prefix : 'O_'?>" size="10">
      , number of digits:
      <select name="term_id_digits" id="term_id_digits">
        <?php 
for ($i=1; $i<=16; $i++) {
?>
        <option value="<?php echo $i?>" <?php  if ($i==7) {?> selected <?php  }?>>
          <?php echo $i?>
          </option>
        <?php 
}
?>
      ...
      ,
      start from:
      <input name="term_id_start" id="term_id_start" value="1" size="10"></td>
  </tr>
  <tr>
